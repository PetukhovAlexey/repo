...
public:
  ...
  MD5(const std::vector<unsigned char>& text);
...
inline MD5::MD5(const std::vector<uint1> &text)
{
    init();
...
inline void MD5::update(const std::vector<unsigned char>& input) {
...
std::vector<unsigned char> MD5::digest() const {
    return std::vector<uint1> (&digest_[0], &digest_[16]);
}
